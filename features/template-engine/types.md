...
| `voiceActivity` | [Число](syntax/expressions.md#primitivy) | Длительность голосовой активности участника \(миллисекунды\) |
...
{% hint style="warning" %}
Функцию `sendMessage` можно использовать не более, чем два раза за шаблон, остальные попытки будут проигнорированы.
{% endhint %}
